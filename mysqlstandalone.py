...
def copy_sqlite_to_mysql():
    try:
        ...
        tables = ["Countries", "Brands", "Products", "URLs", "SKU_URL", "ProductStatus", "Prices", "FailedInserts"]
        ...
        for table in tables:
            ...
            if rows:
                ...
                mysql_query = f"INSERT INTO {table} ({columns_string}) VALUES ({placeholders})"

                # Insert data into MySQL
                mysql_cursor.executemany(mysql_query, rows)
                mysql_conn.commit()

                logging.info(f"Copied {len(rows)} rows from SQLite to MySQL for table {table}")
            else:
                logging.info(f"No data to copy for table {table}")

        logging.info("Data copy from SQLite to MySQL completed successfully")

    except Error as e:
        logging.error(f"Error copying data to MySQL: {e}")
